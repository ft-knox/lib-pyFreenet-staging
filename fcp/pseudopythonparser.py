...
# Firstoff we need a reader which can be given consecutive lines and parse
# these into a dictionary of variables.
...
class Parser:
    ...
    def checkandprocessunprocessed(self):
        ...
        if self.endunparsed in self.unparsedstring:
            self.data[
                self.unparsedvariable] = self.jsonload(
                self.unparsedstring)
            self.unparsed, self.unparsedvariable, self.endunparsed = [], "", ""

    def readline(self, line):
        """Read one line of text."""
        # if we have unparsed code and this line does not end it, we just add
        # the code to the unparsed code.
        if self.unparsed and not self.endunparsed:
            raise ValueError(
                "We have unparsed data but we do not know how it ends. THIS IS A BUG.")
        ...
        if self.unparsed and self.endunparsed and line.strip().endswith(self.endunparsed):
            ...
            # We have to replace those in the content and hope that nothing
            # will break.
        ...
        if True in [i in variable for i in forbiddenvariablechars]:
            raise ValueError(
                "Variables must not contain any of the forbidden characters: " +
                str(forbiddenvariablechars))
